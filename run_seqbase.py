...
def parse_args():
    ...
    # Sanity checks
    if args.dataset_name is None and args.seq_train_file is None and args.seq_val_file is None:
...
def main():
    ...
    if args.seq_train_file is not None or args.dataset_name is not None:
        seq_structure_source = 'seq_train'
    elif args.seq_val_file is not None:
        seq_structure_source = 'seq_validation'
    else:
    ...
    if args.do_eval:  
        tmp_dataset = processed_seq_datasets["seq_validation"]
        for index in random.sample(range(len(tmp_dataset)), 3):
            logger.info(f"Sample {index} of the SEQ validation set: {tmp_dataset[index]}.")
            eg = seq_datasets["seq_validation"][index]
            logger.info(f"Original form: {eg}.")
    ...
    if args.do_train:
        train_dataset = processed_seq_datasets['seq_train']
    ...
    if args.do_eval or args.do_predict: # To allow for predict from another test set
        ...
        eval_aseq_unique_corpus = list(set(eval_aseq_corpus_col))
    else:
        eval_aseq_dataloader = None
    ...
    model, optimizer, aseq_dataloader, eval_aseq_dataloader = accelerator.prepare(
            model, optimizer, aseq_dataloader, eval_aseq_dataloader
    )
    ...
    if args.do_train or args.do_eval:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_train:
                model.train()
                for step, aseq_batch in enumerate(aseq_dataloader):

                    outputs = model(**aseq_batch)

                    loss = outputs.loss
                    loss = loss / args.gradient_accumulation_steps
                    accelerator.backward(loss)

                    if step % args.gradient_accumulation_steps == 0 or step == len(aseq_dataloader) - 1:
                        optimizer.step()
                        lr_scheduler.step()
                        optimizer.zero_grad()
            ...
            if args.do_eval:
                ...
                # Print predictions
                for d in eval_aseq_unique_corpus+['all']:
    ...
    if args.do_predict:
        ...
        # Save Predictions
        eval_aseq_index_col = seq_datasets["seq_validation"]["index"]
